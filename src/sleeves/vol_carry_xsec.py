...
def signals_xsec_volcarry(df_map: dict, top_q: float = 0.35, bot_q: float = 0.35, lookback=21*3) -> dict:
    symbols = list(df_map.keys())
    close = pd.concat([df_map[s]['Close'].rename(s) for s in symbols], axis=1).sort_index().ffill()
    ...
    for s in symbols:
        mask = pd.Series(False, index=close.index); mask.loc[month_end_set] = True
