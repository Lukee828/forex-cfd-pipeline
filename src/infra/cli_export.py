...
def main():
    ...
    for pair in [p.strip() for p in args.pairs.split(",") if p.strip()]:
        tbl = export_risk_snapshot(
            RiskInputs(pair=pair, spread_bps=args.spread_bps), db_path=str(db_path)
        )
        ...
        con.execute(
            f"COPY (SELECT * FROM {tbl} WHERE pair = ? ORDER BY ts DESC LIMIT 1) "
            f"TO ? (FORMAT PARQUET)",
            [pair, str(outfile)],
        )
