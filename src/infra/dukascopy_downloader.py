...
def _normalize(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Missing OHLC columns in downloaded data: {missing}. Got columns: {list(df.columns)}")
...
def _fetch_with_library(symbol: str, start: str, end: str) -> pd.DataFrame:
    ...
    def resolve_instrument(sym: str):
        ...
        for name in dir(inst):
            up = name.upper()
            score = 0
            if all(p in up for p in want_parts): score += 2
            if s in up or "_".join(want_parts) in up: score += 1
            if "FX" in up: score += 0.5
            if score > 0: ranked.append((score, name))
        if not ranked:
            raise SystemExit(f"Could not resolve instrument for '{sym}'. Inspect dukascopy_python.instruments")
