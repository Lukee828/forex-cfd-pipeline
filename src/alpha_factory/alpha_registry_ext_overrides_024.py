...
def _rank(
    self: AlphaRegistry, *, metric: str, filters: Optional[Dict[str, Any]] = None, top_n: int = 100
) -> pd.DataFrame:
...
def _rel_names(con):
    ...
    try:
        names |= {
            r[0].lower() for r in con.execute("SELECT table_name FROM duckdb_tables()").fetchall()
        }
    except Exception:
        pass
    try:
        names |= {
            r[0].lower() for r in con.execute("SELECT table_name FROM duckdb_views()").fetchall()
        }
    except Exception:
...
def _rank(self, *, metric: str, filters=None, top_n: int = 100):
    ...
    if filters:
        ...
        if w:
            clauses.append(str(w))
        t = filters.get("tag")
        if t:
            clauses.append(f"LOWER(CAST(tags AS VARCHAR)) LIKE '%{str(t).lower()}%'")
        s = filters.get("since")
        if s:
            clauses.append(f"CAST(timestamp AS VARCHAR) >= '{s}'")
...
def _choose_cols(con, rel):
    cols = _cols(con, rel)
    alpha_col = (
        "alpha_id" if "alpha_id" in cols else ("config_hash" if "config_hash" in cols else "NULL")
    )
...
def _rank(self, *, metric: str, filters=None, top_n: int = 100):
    ...
    if filters:
        ...
        t = filters.get("tag")
        if t:
            clauses.append(f"LOWER(CAST({tags_col} AS VARCHAR)) LIKE '%{str(t).lower()}%'")
        s = filters.get("since")
        if s:
            clauses.append(f"CAST({ts_expr} AS VARCHAR) >= '{s}'")
    where_sql = " WHERE " + " AND ".join(clauses)
...
def _register_run(self, run: dict) -> str:
    ...
    metrics_txt = (
        ...
        else (metrics if metrics is not None else None)
    )
    ...
    cfg_diff_txt = (
        json.dumps(cfg_diff)
        if isinstance(cfg_diff, (dict, list))
        else (cfg_diff if cfg_diff is not None else None)
    )
