...
def ann_metrics(equity_csv: Path):
    ...
    mar = (
        (cagr / abs(dd))
        if (isinstance(cagr, float) and isinstance(dd, float) and dd < 0)
        else np.nan
    )
...
def main():
    ...
    out_root = (
        Path(args.out_root)
        if args.out_root
        else (ROOT / "runs" / f"robustness_{datetime.now().strftime('%Y%m%d_%H%M')}")
    )
    ...
    for i, (wv_, vct_, vcb_, vclb_, tv_, vlb_, ml_) in enumerate(combos, 1):
        ...
        cmd = [
            ...
            str(vclb_),
            "--out_prefix",
            prefix,  # NEW: isolate outputs per run
        ]
        if args.start:
