...
def backtest_step(
    out_prefix: str, start: str | None = None, end: str | None = None
) -> None:
...
def make_orders_step(nav: float, gross_cap: float, max_price_age_days: int) -> None:
    args = [
        sys.executable,
        "-m",
        "src.exec.make_orders",
        "--positions_csv",
        str(ROOT / "signals" / "positions.csv"),
        "--contracts_csv",
        str(ROOT / "config" / "contracts.csv"),
        "--prices_folder",
        str(ROOT / "data" / "prices_1d"),
        "--out_csv",
        str(ROOT / "signals" / "orders.csv"),
        "--nav",
        str(nav),
        "--gross_cap",
        str(gross_cap),
        "--max_price_age_days",
        str(max_price_age_days),
    ]
    run(args)
...
def parse_args(argv=None):
    ...
    p.add_argument(
        "--mt5_verify_positions",
        action="store_true",
        help="(reserved) include MT5 verification in publisher",
    )
