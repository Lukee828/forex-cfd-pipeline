    ...
    if out_file.exists() and resume and not force:
        ...
        if last_ts is not None:
            ...
            if tf.lower() in ("1d", "d", "daily"):
                eff_start = (last_ts.normalize() + pd.Timedelta(days=1)).strftime(
                    "%Y-%m-%d"
                )
            else:
...
def main():
    ...
    ap.add_argument(
        "--tf", default="1d", choices=["1d", "1h"], help="Timeframe to download"
    )
    ap.add_argument(
        "--start",
        default="2015-01-01",
    ...
    ap.add_argument(
        "--resume", action="store_true", help="Resume from last bar if file exists"
    )
    ap.add_argument(
        "--force",
        action="store_true",
    ...
    ap.add_argument(
        "--dry_run", action="store_true", help="Print what would run without executing"
    )
