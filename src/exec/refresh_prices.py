...
def build_cmd(symbol: str, tf: str, start: str, end: str, out_file: Path) -> list[str]:
    return [
        sys.executable, "-m", "src.data.dukascopy_downloader",
        "--symbol", symbol,
        "--tf", tf,
        "--start", start,
        "--end", end,
...
def worker(symbol: str, tf: str, start: str, end: str, out_folder: Path, resume: bool, force: bool, dry_run: bool) -> tuple[str, str]:
    ...
    if cp.returncode != 0:
        return (symbol, f"ERROR code={cp.returncode}\nSTDOUT:\n{cp.stdout}\nSTDERR:\n{cp.stderr}")
    return (symbol, f"OK → {out_file.name} [{eff_start}→{eff_end}]")

def main():
    ap = argparse.ArgumentParser(description="Refresh Dukascopy price files for all symbols in contracts.csv")
    ap.add_argument("--contracts_csv", default="config/contracts.csv", help="Path to contracts spec with 'symbol' column")
    ap.add_argument("--tf", default="1d", choices=["1d","1h"], help="Timeframe to download")
    ap.add_argument("--start", default="2015-01-01", help="Start date YYYY-MM-DD (ignored if --resume finds newer)")
    ap.add_argument("--end", default=None, help="End date YYYY-MM-DD (default: today UTC)")
    ap.add_argument("--out_folder", default=None, help="Override output folder (default: data/prices_1d or prices_1h)")
    ap.add_argument("--resume", action="store_true", help="Resume from last bar if file exists")
    ap.add_argument("--force", action="store_true", help="Force full re-download even if file exists")
    ...
    contracts_csv = (ROOT / args.contracts_csv) if not Path(args.contracts_csv).is_absolute() else Path(args.contracts_csv)
    ...
    print(f"Symbols: {len(symbols)} | TF: {args.tf} | Out: {out_folder} | Start: {args.start} | End: {end}")
    print(f"Options: resume={args.resume} force={args.force} dry_run={args.dry_run} workers={args.max_workers}")

    futures = []
    with ThreadPoolExecutor(max_workers=args.max_workers) as ex:
        for sym in symbols:
            futures.append(ex.submit(worker, sym, args.tf, args.start, end, out_folder, args.resume, args.force, args.dry_run))
