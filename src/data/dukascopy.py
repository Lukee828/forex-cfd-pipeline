...
def _to_pandas_freq(tf: str) -> str:
    t = (tf or "").upper().strip()
    mapping = {"M1":"1T","M5":"5T","M15":"15T","M30":"30T","H1":"1H","H4":"4H","D1":"1D"}
    if t in mapping:
        return mapping[t]
    m = re.fullmatch(r"([MHD])(\d+)", t)
    if m:
        unit, num = m.group(1), m.group(2)
        return f"{num}{'T' if unit=='M' else unit}"
    return t
...
def _offline_bars(spec: BarSpec) -> pd.DataFrame:
    ...
    df = pd.DataFrame({        'open': price,        'high': price,        'low': price,        'close': price    }, index=idx)
    df.index.name = "ts"
    return df
...
def get_bars(spec: BarSpec) -> pd.DataFrame:
    ...
    # TODO: live Dukascopy download; for now, fall back to offline for smoke
    return _offline_bars(spec)
