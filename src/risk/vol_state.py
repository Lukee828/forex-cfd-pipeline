...
def rolling_vol(
    close: pd.Series, window: int = 20, freq_hint: Optional[str] = None
) -> pd.Series:
    """
...
class VolStateMachine:
    ...
    def fit(self, close: pd.Series) -> "VolStateMachine":
        ...
        if not (self.low_hi <= self.med_hi):
            self.low_hi, self.med_hi = min(self.low_hi, self.med_hi), max(
                self.low_hi, self.med_hi
            )
        return self
...
def infer_vol_regime(
    ...
    """
    vsm = VolStateMachine(
        window=window, lower_q=lower_q, upper_q=upper_q, freq_hint=freq_hint
    )
    vsm.fit(close)
