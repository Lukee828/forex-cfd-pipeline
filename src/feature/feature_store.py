...
def _fs_record_provenance(self, *args, **kwargs):
    ...
    try:
        ...
        with log.open("a", encoding="utf-8") as f:
            f.write(
                _json.dumps({"id": new, "args": list(args), "kwargs": kwargs}) + "\n"
            )
    except Exception:
        pass
    return new
...
def _fs_exists_final(self, key, maybe_digest=None):
    ...
    return (
        _P(maybe_digest).exists()
        if maybe_digest is not None
        else self._path_for_key(key).exists()
    )
    ...
    def _infer_schema_if_df(val):
        try:
            ...
            if isinstance(val, _pd.DataFrame):
                ...
                return {
                    "columns": [[str(c), str(val.dtypes[c])] for c in list(val.columns)]
                }
        except Exception:
            pass
        return None
