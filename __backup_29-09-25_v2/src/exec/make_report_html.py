...
def make_report(equity_csv=None, attrib_csv=None, monthly_csv=None, out_html=None,
    ...
    if robustness_summary_csv and Path(robustness_summary_csv).exists():
        ...
        if {'maxdd','sharpe'}.issubset(sdf.columns):
            ...
            pareto_html = pf[cols].to_html(index=False, float_format=lambda x: f"{x:.4f}" if isinstance(x,float) else str(x))
...
def main():
    ...
    make_report(args.equity_csv, args.attrib_csv, args.monthly_csv, args.out_html,
                robustness_summary_csv=args.robustness_summary_csv, pareto_top_n=args.pareto_top_n)
