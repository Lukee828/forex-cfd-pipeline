...
def _load_many(paths):
    ...
    for p in paths:
        ...
        sym = (
            ...
            else pathlib.Path(p).stem.upper()
        )
...
def _mtd_gate_proxy(dfs, soft=-0.06, hard=-0.10):
    ...
    panel = (
        pd.concat([dfs[s]["Close"].rename(s) for s in dfs], axis=1).sort_index().ffill()
    )
...
def main():
    ...
    for s in dfs.keys():
        intents = ts_signals_trend(
            df_d=dfs[s].assign(symbol=s),
        ...
        for oi in intents:
            if oi.symbol == s:
                trend[s].loc[pd.Timestamp(oi.ts_utc)] = (
                    1.0 if oi.side == "long" else -1.0
                )
    ...
    if args.equity_csv and os.path.exists(args.equity_csv):
        try:
            gate_mult = _mtd_gate_from_equity(
                args.equity_csv, args.mtd_soft, args.mtd_hard
            )
        except Exception:
    ...
    for s, df in dfs.items():
        v = (
            trend[s].loc[asof] * weights["tsmom"]
            + xsec[s].reindex([asof], method="ffill").fillna(0.0).iloc[0]
            * weights["xsec"]
        ...
        vol = df["Close"].pct_change().rolling(
            args.vol_lookback, min_periods=max(5, args.vol_lookback // 2)
        ...
        rows.append(
            {
                ...
                "symbol": s,
                "side": side,
                "leverage": round(float(lev), 6),
                "gross_usd": round(args.nav * float(lev), 2),
            }
        )
