...
def infer_tf_from_path(path: pathlib.Path) -> str:
    ...
    if 'prices_1m' in name: return '1m'
    if 'prices_5m' in name: return '5m'
    if 'prices_1h' in name: return '1h'
    if 'prices_1d' in name: return '1d'
    return '?'
...
def validate_file(pq: pathlib.Path) -> dict:
    ...
    nans = {c:int(df[c].isna().sum()) for c in ['Open','High','Low','Close','Volume']}
...
def main():
    ...
    ap.add_argument('--root', default='data', help='Root folder with prices_* subfolders')
    ap.add_argument('--max-files', type=int, default=20, help='Limit files for quick scan')
    args = ap.parse_args()
