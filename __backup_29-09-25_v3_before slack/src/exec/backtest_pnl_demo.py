...
def main():
    ...
    if args.start or args.end:
        ...
        for s, df in list(panel.items()):
            ...
            if start_ts and end_ts:
                df = df.loc[(idx >= start_ts) & (idx <= end_ts)]
            elif start_ts:
                df = df.loc[idx >= start_ts]
            elif end_ts:
                df = df.loc[idx <= end_ts]
    ...
    eq.to_csv(f"data/{prefix}_equity.csv")
    if len(trades):
    ...
    try:
        ...
        if "position" in pos.columns:
            pos = pos[["symbol", "position"]]
        elif "qty" in pos.columns:
            pos = pos[["symbol"]].assign(position=pos["qty"].clip(-1, 1))
        elif "side" in pos.columns:
            side_map = {"long": 1.0, "buy": 1.0, "short": -1.0, "sell": -1.0}
            pos = pos[["symbol"]].assign(
                position=pos["side"].astype(str).str.lower().map(side_map).fillna(0.0)
            )
        else:
            pos = pos[["symbol"]].assign(position=0.0)

        pos["asof_utc"] = datetime.now(timezone.utc).isoformat()

        out_positions = Path("data") / f"{args.out_prefix or 'pnl_demo'}_positions.csv"
        out_positions.parent.mkdir(parents=True, exist_ok=True)
        pos.to_csv(out_positions, index=False)
        print("Saved positions snapshot to", out_positions)
