...
import argparse, subprocess, sys, json, math, re
...
def pick_best_is(summary_csv: Path):
    ...
    df = df.sort_values(["sharpe","absdd","vol","mar"], ascending=[False, True, True, False])
...
def _json_loads_tolerant(s: str):
    ...
    try:
        ...
        raise SystemExit(f"Failed to parse JSON for --folds/--folds_file:\n{s}\nError: {e}")
...
def main():
    ...
    grp.add_argument("--folds", help='JSON list, or prefix with @file, e.g. --folds @runs\\wf_folds.json')
    ...
    ap.add_argument("--autofreeze", dest="autofreeze", action="store_true", default=True,
                    help="After finishing, write best params into production.yaml (default: on)")
    ap.add_argument("--no-autofreeze", dest="autofreeze", action="store_false",
                    help="Disable auto-write to production.yaml")
    ap.add_argument("--prod_yaml", default=str(ROOT/"config"/"production.yaml"),
    ...
    for k, f in enumerate(folds, 1):
        ...
        # 1) IS sweep
        cmd_is = [
            sys.executable, "-m", "src.exec.sweep_robustness",
            "--folder", args.folder, "--costs_csv", args.costs_csv,
            "--start", f["is_start"], "--end", f["is_end"],
            "--w_volcarry_grid", args.w_volcarry_grid,
            "--volcarry_topq_grid", args.volcarry_topq_grid,
            "--volcarry_botq_grid", args.volcarry_botq_grid,
            "--volcarry_lookback_grid", args.volcarry_lookback_grid,
            "--target_vol_grid", args.target_vol_grid,
            "--vol_lookback_grid", args.vol_lookback_grid,
            "--max_lev_grid", args.max_lev_grid,
            "--limit", str(args.limit),
            "--out_root", str(is_dir)
        ]
        print("RUN IS:", " ".join(cmd_is)); run(cmd_is)
        ...
        cmd_oos = [
            ...
            "--folder", args.folder, "--costs_csv", args.costs_csv,
            "--start", f["oos_start"], "--end", f["oos_end"],
            "--target_ann_vol", str(best.get("target_vol", 0.12)),
            "--vol_lookback",  str(int(best.get("vol_lookback", 20))),
            "--max_leverage",  str(best.get("max_leverage", 3.0)),
            "--w_tsmom", "1.0", "--w_xsec", "0.8", "--w_mr", "0.6",
            "--w_volcarry",    str(best.get("w_volcarry", 0.0)),
            "--volcarry_top_q", str(best.get("vc_top_q", 0.35)),
            "--volcarry_bot_q", str(best.get("vc_bot_q", 0.35)),
            "--volcarry_lookback", str(int(best.get("vc_lookback", 63))),
            "--out_prefix", prefix
        ]
        print("RUN OOS:", " ".join(cmd_oos)); run(cmd_oos)
        ...
        row = {
            ...
            "is_sharpe": is_metrics.get("sharpe"), "is_maxdd": is_metrics.get("maxdd"),
            "oos_sharpe": oos_metrics.get("sharpe"), "oos_maxdd": oos_metrics.get("maxdd"),
            "params": json.dumps({
    ...
    if args.autofreeze:
        freeze_cmd = [
            sys.executable, "-m", "src.exec.freeze_params",
            "--wf_summary", str(out_csv),
            "--prod_yaml", args.prod_yaml
        ]
