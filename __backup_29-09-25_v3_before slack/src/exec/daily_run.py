...
def run_backtest(folder, costs_csv, start, end, params, out_prefix):
    cmd = [
        ...
        "--target_ann_vol", str(params['risk']['target_ann_vol']),
        "--vol_lookback",  str(params['risk']['vol_lookback']),
        "--max_leverage",  str(params['risk']['max_leverage']),
        "--w_tsmom",       str(params['weights'].get('tsmom', 1.0)),
        "--w_xsec",        str(params['weights'].get('xsec', 0.8)),
        "--w_mr",          str(params['weights'].get('mr', 0.6)),
        "--w_volcarry",    str(params['weights'].get('volcarry', 0.0)),
        "--volcarry_top_q",     str(params['volcarry'].get('top_q', 0.35)),
        "--volcarry_bot_q",     str(params['volcarry'].get('bot_q', 0.35)),
        "--volcarry_lookback",  str(params['volcarry'].get('lookback', 63)),
        "--mtd_soft",      str(params['mtd_gates'].get('soft', -0.06)),
        "--mtd_hard",      str(params['mtd_gates'].get('hard', -0.10)),
        "--gap_atr_k",     str(params['trade_sanity'].get('gap_atr_k', 3.0)),
        "--atr_lookback",  str(params['trade_sanity'].get('atr_lookback', 14)),
        "--vol_spike_mult",str(params['trade_sanity'].get('vol_spike_mult', 3.0)),
        "--vol_spike_window", str(params['trade_sanity'].get('vol_spike_window', 60)),
...
def make_positions_from_trades(trades_csv: Path, out_csv: Path):
    import numpy as np
    tr = pd.read_csv(trades_csv, parse_dates=['exit_time','entry_time'], dayfirst=False)
    ...
    if pos_series is None:
        ...
        pd.DataFrame(columns=['symbol','position','asof_utc']).to_csv(out_csv, index=False)
        print("Trades CSV lacks position/qty/side/dir; wrote flat positions:", out_csv)
        return

    tr = tr.assign(_position=pos_series)
    order_col = 'exit_time' if 'exit_time' in tr.columns else ('entry_time' if 'entry_time' in tr.columns else None)
    if order_col:
        tr = tr.sort_values(['symbol', order_col])
    last = tr.groupby('symbol', as_index=False).tail(1)[['symbol','_position']].rename(columns={'_position':'position'})
...
def main():
    ...
    start = (asof - pd.Timedelta(days=365*int(params['universe'].get('lookback_years',5)))).strftime("%Y-%m-%d")
    ...
    positions_csv = ROOT / params.get('output',{}).get('positions_csv', 'signals/positions.csv')
    ...
    mtd = (eq_month['portfolio_equity'].iloc[-1] / eq_month['portfolio_equity'].iloc[0] - 1)
    hard = float(params['mtd_gates'].get('hard', -0.10))
    if mtd <= hard:
        print(f"MTD hard stop breached ({mtd:.2%} <= {hard:.2%}); writing zero positions.")
        positions_csv.parent.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(columns=['symbol','position','asof_utc']).to_csv(positions_csv, index=False)
        return
    ...
    if mtd <= hard:
        ...
        send_email(subject=f"[DAILY] MTD HARD BREACH {out_prefix}", body_text=body_txt, body_html=body_html)
        return
    ...
    send_email(subject=f"[DAILY] run OK {out_prefix}", body_text=body_txt, body_html=body_html)
...
if __name__ == "__main__":
    try:
        ...
        try:
            send_email(subject="[DAILY] run FAILED", body_text=str(e), body_html=f"<pre>{e}</pre>")
        except Exception:
            pass
        raise
