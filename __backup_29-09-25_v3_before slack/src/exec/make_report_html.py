...
def load_summary(path):
    ...
    if p and p.exists():
        ...
        for c in ['run','loo','lookbacks','w_tsmom','w_xsec','w_mr','w_volcarry',
                  'vc_top_q','vc_bot_q','vc_lookback','target_vol','vol_lookback',
                  'max_leverage','cagr','vol','sharpe','maxdd','mar']:
...
# ---------- report ----------
def make_report(equity_csv=None, attrib_csv=None, monthly_csv=None, out_html=None,
                robustness_summary_csv=None, pareto_top_n=10,
                robustness_summary_csv_is=None, robustness_summary_csv_oos=None):
    ...
    out_html = out_html or (ROOT/"reports"/f"report_{datetime.now().strftime('%Y%m%d_%H%M')}.html")
    ...
    if not df_is.empty or not df_oos.empty:
        ...
        if not df_is.empty:
            ...
            if not pf_is.empty:
                pareto_html_is = pf_is[select_cols(pf_is)].to_html(index=False, float_format=lambda x: f"{x:.4f}" if isinstance(x,float) else str(x))
        if not df_oos.empty:
            pf_oos = pareto_front(df_oos, xcol='maxdd', ycol='sharpe', top_n=pareto_top_n)
            if not pf_oos.empty:
                pareto_html_oos = pf_oos[select_cols(pf_oos)].to_html(index=False, float_format=lambda x: f"{x:.4f}" if isinstance(x,float) else str(x))

        # overlay scatter Sharpe vs |MaxDD|
        if not df_is.empty or not df_oos.empty:
            fig_sc = go.Figure()
            if not df_is.empty:
                d1 = df_is.copy(); d1['|maxdd|'] = d1['maxdd'].abs()
                fig_sc.add_trace(go.Scatter(
                    x=d1['|maxdd|'], y=d1['sharpe'],
                    mode='markers', name='IS',
                    hovertext=[str(r) for _, r in d1[select_cols(d1)].iterrows()]
                ))
            if not df_oos.empty:
                d2 = df_oos.copy(); d2['|maxdd|'] = d2['maxdd'].abs()
                fig_sc.add_trace(go.Scatter(
                    x=d2['|maxdd|'], y=d2['sharpe'],
                    mode='markers', name='OOS',
        ...
        if not df_single.empty:
            ...
            if not pf.empty:
                pareto_html_single = pf[select_cols(pf)].to_html(index=False, float_format=lambda x: f"{x:.4f}" if isinstance(x,float) else str(x))
...
def main():
    ...
    make_report(args.equity_csv, args.attrib_csv, args.monthly_csv, args.out_html,
