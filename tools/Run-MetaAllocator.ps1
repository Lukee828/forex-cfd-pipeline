...
try {
  if ($Demo -or -not (Test-Path $Csv)) {
    ...
    Set-Content -LiteralPath $tmp -Encoding UTF8 -Value @(
      ...
      "rows = pd.DataFrame({",
      "  'sharpe':[1.2,0.8,0.3], 'dd':[0.10,0.05,0.20]}, index=['ZZ','S2E','MR'])",
    ...
    Set-Content -LiteralPath $tmp -Encoding UTF8 -Value @(
      ...
      "import pandas as pd",
      "from src.infra.meta_allocator import MetaAllocator, MetaAllocatorConfig",
      "df = pd.read_csv(r'$Csv').set_index(df.columns[0]) if 'index' not in df.columns else pd.read_csv(r'$Csv').set_index('index')",
