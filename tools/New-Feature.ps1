...
# --- Auto Repair-PR integration (non-interactive) ---
try {
    $mergeState = gh pr view $prNum --json mergeable,mergeStateStatus -q ".mergeStateStatus" 2>$null
    if ($mergeState -match "CONFLICTING|DIRTY") {
        Write-Host "⚠ PR #$prNum not clean ($mergeState) — auto-repairing..." -ForegroundColor Yellow
        pwsh -NoProfile -File "tools/Repair-PR.ps1" -Number $prNum
    }
} catch {
    Write-Host "⚠ Repair-PR helper failed or missing: $(param(
  [Parameter()][string]$Branch = "",
  [Parameter(Mandatory)][string]$Title,
  [Parameter(Mandatory)][string]$Notes,
  [string]$Tag = ""
...
# Optional tag & release# Optional tag & release
if ($Tag) {
  git switch main; git pull --ff-only
  if (-not (git tag -l $Tag)) {
    git tag $Tag; git push origin $Tag
