...
def _read_prices_1d(symbol: str) -> pd.DataFrame:
    ...
    if pq.exists():
        ...
        raise FileNotFoundError(f"Missing price file for {symbol} in {base} (.parquet or .csv).")
    ...
    if close is None:
        raise ValueError(f"{symbol}: cannot locate Close column; cols={list(df.columns)}")
...
def _load_cfg(path: str) -> dict:
    ...
    if not core and not sat:
        raise ValueError("No symbols specified in config.symbols.core or symbols.satellite.")
...
def _portfolio_returns(close_panel: pd.DataFrame) -> pd.Series:
    ...
    w = pd.DataFrame(np.where(~rets.isna(), 1.0, np.nan), index=rets.index, columns=rets.columns)
...
def main():
    ...
    print(f"Total: {total_ret:.2%}  AnnRet: {ann:.2%}  AnnVol: {vol:.2%}  Sharpe: {sharpe:.2f}")
    if missing:
