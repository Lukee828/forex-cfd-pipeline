...
class RetryPolicy:
    def __init__(self, attempts: int = 3, sleep_sec: float = 0.4):
        self.attempts = attempts; self.sleep_sec = sleep_sec

class MT5:
    def __init__(self, terminal_path: Optional[str] = None, retry: RetryPolicy = RetryPolicy()):
        self.terminal_path = terminal_path; self.retry = retry
    def _init(self):
    ...
    def _shutdown(self): mt5.shutdown()
    def _with_retry(self, fn):
        ...
        for i in range(self.retry.attempts):
            try:
                ...
                last=e
                if i==self.retry.attempts-1: raise
                time.sleep(self.retry.sleep_sec)
        raise last or RuntimeError("Unknown MT5 error")

    @staticmethod
    def _df_from_rates(rows)->pd.DataFrame:
        if rows is None: return pd.DataFrame(columns=["open","high","low","close","tick_volume","spread","real_volume"])
    ...
    def parse_iso(dt: Optional[str])->Optional[datetime]:
        ...
        try:
            if len(dt)==10: return _dt.strptime(dt,"%Y-%m-%d")
    ...
    def tick(self, symbol:str)->Optional[Dict[str,Any]]:
        def inner():
            ...
            return getattr(t,"_asdict",lambda:{k:getattr(t,k) for k in dir(t) if not k.startswith("_")})()
    ...
    def copy_rates_range_df(self, symbol:str, timeframe:str, dt_from:str, dt_to:str)->pd.DataFrame:
        tf = TF.get(timeframe.upper())
        if tf is None: raise ValueError(f"Unsupported timeframe: {timeframe}")
        f = self.parse_iso(dt_from); t = self.parse_iso(dt_to)
        if not (f and t): raise ValueError("Both dt_from and dt_to required")
        def inner():
            mt5.symbol_select(symbol, True)
    ...
    def positions(self, symbol: Optional[str]=None):
        def inner():
            ...
            return [getattr(x,"_asdict",lambda:{k:getattr(x,k) for k in dir(x) if not k.startswith("_")})() for x in res]
        return self._with_retry(inner)
...
def _cli():
    ...
    ap.add_argument("--mode", choices=["ticks","rates","rates_range","positions","orders"], required=True)
    ...
    if a.mode == "ticks":
        ...
        print(json.dumps(MT5().positions(), ensure_ascii=False)); return
    elif a.mode == "orders":
        print(json.dumps(MT5().orders(), ensure_ascii=False)); return
    elif a.mode == "rates":
        if len(a.symbols)!=1: print("ERROR: --mode rates needs one symbol", file=sys.stderr); sys.exit(2)
        df = api.copy_rates_df(a.symbols[0], timeframe=a.timeframe, count=a.count)
    else: # rates_range
        if len(a.symbols)!=1: print("ERROR: --mode rates_range needs one symbol", file=sys.stderr); sys.exit(2)
        if not (a.dt_from and a.dt_to): print("ERROR: rates_range needs --from and --to", file=sys.stderr); sys.exit(2)
        df = api.copy_rates_range_df(a.symbols[0], timeframe=a.timeframe, dt_from=a.dt_from, dt_to=a.dt_to)

    if a.outparquet:
        df.to_parquet(a.outparquet, index=True); print("WROTE_PARQUET", os.path.abspath(a.outparquet)); return
    if a.outcsv:
        df.to_csv(a.outcsv); print("WROTE_CSV", os.path.abspath(a.outcsv)); return
    print(df.to_csv())
